...
rule summaryreport:
  input:vcf="output/{dataset}/mapping/gatk3/realign_all_samples.all_sites.vcf.gz",
        ...
        inv="output/{dataset}/mapping/gatk3/inversion_mips/inversion_summary_counts.txt",
...
rule gatk4_gvcfs:
  ...
  conda:"python3gatk4.yml"
  shell:"for i in $(less {input.lst} | cut -f 2 ); do (gatk HaplotypeCaller --max-reads-per-alignment-start 0 --disable-optimizations --sample-name ${i} -ERC GVCF -R {input.fasta} -I {input.bam} -L {targets} -O {output} ); done"
...
rule gatk4_combine:
  ...
  conda:"python3gatk4.yml"
  shell:"gatk CombineGVCFs --break-bands-at-multiples-of 1 -R {fasta} $(ls -1 {input.gvcf} | xargs -n 1 echo -V ) -O {output}"
...
rule summaryreport_gatk4:
  input:vcf="output/{dataset}/mapping/gatk4/realign_all_samples.all_sites.vcf.gz",
        ...
        inv="output/{dataset}/mapping/gatk4/inversion_mips/inversion_summary_counts.txt",
        sex="output/{dataset}/mapping/samples_sex_check.txt",
        target="input/{dataset}target_coords.bed",
