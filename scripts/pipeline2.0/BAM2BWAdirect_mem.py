...
Align reads stored as BAM using BWA MEM
...
parser.add_option("--bwa_params", dest="bwa_params", help="Parameters passed to bwa (default '-L 80 -M')",default="-L 80 -M")
...
try:
  ...
  #FORWARD READ EXTRACTION
  cjob = options.bwa_bin+" mem -b1 %s -t %d %s/%s %s"%(options.bwa_params,options.cores,options.bwa_genome,options.indexName,filename)
  if options.mock:
    sys.stderr.write(cjob+"\n")
  else:
    forward = subprocess.Popen(cjob.split(),bufsize=0,stdout=subprocess.PIPE)

  #REVERSE READ EXTRACTION
  ...
  cjob = options.bwa_bin+" mem %s %s/%s /dev/fd/%d /dev/fd/%d %s %s"%(options.bwa_params,options.bwa_genome,options.indexName,forward.stdout.fileno(),reverse.stdout.fileno(),filename,filename)
  if options.mock:
    sys.stderr.write(cjob+"\n")
  else:
